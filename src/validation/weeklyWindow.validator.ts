...
export class WeeklyWindowValidator {
  static validateWeeklyWindow(weeklyWindow: WeeklyWindow): string | null {
    const [startTimeError, startTime] = WeeklyWindowValidator.validateTime(
      weeklyWindow.start_time,
    );
    if (startTimeError !== null) {
      return startTimeError;
    }
    const [endTimeError, endTime] = WeeklyWindowValidator.validateTime(
  ...
  static validateTime(time: string): [string, null] | [null, [number, number]] {
